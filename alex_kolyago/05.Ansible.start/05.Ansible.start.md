...
Vagrant.configure("2") do |config|
  config.vm.define "ubuntu20" do |ubuntu|
    ...
    ubuntu.vm.provision "shell", run: "always", inline: <<-SHELL
        sudo useradd updater -p ansible
...
```
```
vagrant@ubuntu-focal:~$ ansible localhost -m shell -a "sudo apt upgrade -y"
...
```bash
vagrant@ubuntu-focal:~$ ansible ec_all -m ping
work_02 | SUCCESS => {
    "ansible_facts": {
...
vagrant@ubuntu-focal:~$ ansible ec_all -m shell -a "hostname && hostname -i"
...
vagrant@ubuntu-focal:~$ ansible work_02 -m shell -a "apt update && apt upgrade -y"
work_02 | CHANGED | rc=0 >>
...
vagrant@ubuntu-focal:~$ ansible work_01 -m shell -a "yum upgrade -y"
work_01 | CHANGED | rc=0 >>
...
```bash
vagrant@ubuntu-focal:~$ cat /etc/ansible/hosts 
ec_all:
  vars:
...
vagrant@ubuntu-focal:~$ cat /etc/ansible/group_vars/ec_all.yaml
env: "it-academy"
ansible_ssh_common_args: '-o ProxyCommand="ssh -W %h:%p -q jump_sa@178.124.206.53"'

vagrant@ubuntu-focal:~$ ansible-inventory --graph
@all:
  |--@ec_all:
  |  |--@k8s:
  |  |  |--k8s_01

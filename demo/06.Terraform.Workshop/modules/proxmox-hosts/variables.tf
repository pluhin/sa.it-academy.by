...
variable "env_host_count" {
  ...
  validation {
    condition     = var.env_host_count >= 0 && var.env_host_count <= 5
    error_message = "env_host_count must be 0..5: the onboarding grants VM.Allocate on 5 VMIDs per type."
  }
}
...
variable "k3s_node_count" {
  ...
  validation {
    condition     = var.k3s_node_count >= 0 && var.k3s_node_count <= 5
    error_message = "k3s_node_count must be 0..5: the onboarding grants VM.Allocate on 5 VMIDs per type."
  }
}
...
variable "k8s_node_count" {
  ...
  validation {
    condition     = var.k8s_node_count >= 0 && var.k8s_node_count <= 5
    error_message = "k8s_node_count must be 0..5: the onboarding grants VM.Allocate on 5 VMIDs per type."
  }
}

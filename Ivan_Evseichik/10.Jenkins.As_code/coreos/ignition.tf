...
// jenkins users
data "ignition_user" "example_jenkins_user" {
    name = "jenkins"
...
data "template_file" "example_virtual_machine_network_content" {
  ...
  vars = {
    address = "${var.virtual_machine_network_address}"
    gateway = "${var.virtual_machine_gateway}"
    dns     = "${var.virtual_machine_dns_servers}"  
}
}
...
data "ignition_networkd_unit" "example_virtual_machine_network_unit" {
  ...
  content = "${data.template_file.example_virtual_machine_network_content.rendered}"
}
...
data "ignition_file" "example_virtual_machine_hostname_file" {
  ...
  content {
    content = "${var.virtual_machine_name_prefix}.${var.virtual_machine_domain}"
  }
}
...
data "ignition_config" "example_ignition_config" {
  files    = ["${data.ignition_file.example_virtual_machine_hostname_file.id}"]
  systemd  = ["${data.ignition_systemd_unit.example_service_unit.id}"]
  networkd = ["${data.ignition_networkd_unit.example_virtual_machine_network_unit.id}"]

  users = [
    "${data.ignition_user.example_root_user.id}",
    "${data.ignition_user.example_core_user.id}",
    "${data.ignition_user.example_jenkins_user.id}",

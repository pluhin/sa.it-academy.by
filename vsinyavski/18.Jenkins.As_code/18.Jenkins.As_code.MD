...
pipeline {
    ...
    stages {
        stage('Git repo connection') {
            steps {
            git branch: 'main', credentialsId: 'git_ssh_jenkins', url: 'git@github.com:astarosh87/my_repo.git'
            }   
        }
            stage('Count hosts & measure connection speed') {
            steps {
                sh """
                sudo apt update
                ...
                nmap -sn 192.168.201.* 192.168.202.* 192.168.203.* | tail -n 1 > Report.txt
